...
export default function SayingsPhotoFilterion({ cates, attrs }) {
    ...
    if (localStorage.getItem("sayingPhotoAttrDefault")) {
        let getdefaultAttrs = JSON.parse(localStorage.getItem("sayingPhotoAttrDefault"))
    ...
    const onChooseCate = (e) => {
        ...
        localStorage.setItem("sayingPhotoAttr", JSON.stringify(path))
        ...
        localStorage.setItem("sayingPhotoAttrDefault", JSON.stringify(defaultAttrs))
        getData()
    }
    ...
    const onHandelYear = (updatedSelectedDate = selectedDate) => {
        ...
        const path = `${selectedCate ? "category_id=" + selectedCate.id + "&" : ""}${dateChoose ? dateChoose + "&" : ""
            }${wordSearchFormat ? wordSearchFormat + "&" : ""}${selectedTxtFormat ? selectedTxtFormat + "&" : ""}${selectedTAFormat ? selectedTAFormat + "&" : ""
            }`.slice(0, -1);
    ...
    useEffect(() => {
        ...
        if (JSON.parse(localStorage.getItem("sayingPhotoAttr")) === "") {
            localStorage.removeItem("sayingPhotoAttr");
            localStorage.removeItem("sayingPhotoAttrDefault");
        }
    ...
    }, [localStorage.getItem("sayingPhotoAttr")]);
    ...
    return (
        <div className='mediaFiltertion'>
            ...
            {
                localStorage.getItem("sayingPhotoAttr") || localStorage.getItem("sayingPhotoAttrDefault") ?
                    <button className='clearFilterBtn more_btn clearFilterSaying' onClick={() => clearFilter()}>مسح الكل</button>
                    :
                    ""
