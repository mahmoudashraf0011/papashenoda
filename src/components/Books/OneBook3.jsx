...
import "./BooksSearchMoreMainRes.css"
...
/* export default function OneBook({ info, src, book ,book_id,slug}) {
    ...
    return (
        <div className="books-section-one-books-info">
            <div className="books-section-one-info">
                <div className='books-section-one-save'  style={{ direction:"rtl",marginTop:"20%"}}><p><a href={`/displayBook/${slug}`} onClick={getBook} style={{ cursor:"pointer" }} target='_blank'>{info}</a></p></div>
                <p className='books-section-one-save2'  style={{ direction:"rtl" }}><a href={`/displayBook/${slug}`} onClick={getBook} style={{ cursor:"pointer" }} target='_blank'>{info}</a> </p>
                <div className="book-links">
                     <a href={`/displayBook/${slug}`} onClick={getBook}
                     className="download-link books-section-one-download books-section-one-download-two" target='_blank'>
                        قراءة 
                    </a>
                    <a href={book} className="download-link books-section-one-download" style={{marginBottom:"50%"}}> تحميل </a>
...
export default function OneBook3({ info, src, book, book_id, slug }) {
  const [showReader, setShowReader] = useState(false);
  ...
  return (
    <div className="books-section-one-books-info" style={{marginRight:"20%"}}>
      <div className="books-section-one-info">
        ...
        {/* <div className='books-section-one-save'
          style={{
            direction: "rtl",
            marginTop: "11%",
            cursor: "pointer",
      ...
      <a href={`/displayBook/${slug}`} target='_blank'>
        {/* <img
          src={src}
          alt=""
          className={`books-book ${src === "/assets/default/books/Book - inside.png"?"one-book-res":""}`}
          onClick={getBook}
          onError={(e) => {
            e.target.onerror = null; // Prevents looping if default image also fails
            e.target.src = "/assets/default/books/Book - inside.png"; // Replace with your fallback image path
  ...
  style={{
    cursor: "pointer", 
    marginBottom: "10%",
    width: src === "/assets/default/books/Book - inside.png" ? "305px" : src !== "/assets/main/books/Book - inside.png" ? "305px" : "auto",
    height: src === "/assets/default/books/Book - inside.png" ? "265px" : src !== "/assets/main/books/Book - inside.png" ? "245px" : "auto",
    marginTop: src === "/assets/default/books/Book - inside.png" ? "10%" : src !== "/assets/main/books/Book - inside.png" ? "15%" : "auto",
    objectFit: "cover"
  }}
/>
